    ...
    <form className="add-employee-form" onSubmit={handleSubmit}>
      ...
      {Object.keys(formData.currentAddress).map((key) => (
        <div key={key}>
          ...
          <input
            name={key}
            value={formData.currentAddress?.[key as keyof typeof formData.currentAddress] || ''}
            onChange={(e) => handleNestedChange('currentAddress', key, e.target.value)}
          />
        </div>
      ))}
      ...
      {Object.keys(formData.idCardAddress).map((key) => (
        <div key={key}>
          ...
          <input
            name={key}
            value={formData.idCardAddress?.[key as keyof typeof formData.idCardAddress] || ''}
            onChange={(e) => handleNestedChange('idCardAddress', key, e.target.value)}
          />
        </div>
      ))}
